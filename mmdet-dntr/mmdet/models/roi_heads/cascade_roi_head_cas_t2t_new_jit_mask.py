...
class Cascade_t2t_new_jit_mask_RoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        self.token_dim = 100
        self.embed_dim = 128
        self.bbox_token = [nn.Parameter(torch.zeros(1, self.in_chans * 3 * 3)).cuda() for _ in range(self.num_stages)]
        self.cls_token = [nn.Parameter(torch.zeros(1, self.in_chans * 3 * 3)).cuda() for _ in range(self.num_stages)]
        ...
        # box_fc_head = nn.Sequential(nn.Linear(9*128, 512), nn.LeakyReLU(0.1, True), nn.Linear(512, 4))
        # self.t2t_bbox_head = ModuleList([box_fc_head for _ in range(self.num_stages)])
        self.t2t_bbox_head = ModuleList([nn.Linear(9*128, 4) for _ in range(self.num_stages)])

        # print("self.t2t_bbox_head:",self.t2t_bbox_head)
        # exit()
        # norm_layer = partial(nn.LayerNorm, eps=1e-6)
        ## do the bipartite token pair ##
        self.token_pair = ModuleList([Token_pair_block(dim=self.embed_dim, num_heads=num_head)  for _ in range(self.num_stages)]) ## 8
        ...
        ###### aitod dataset #####
        # cls_fc_head = nn.Sequential(nn.Linear(9*128, 512), nn.LeakyReLU(0.1, True), nn.Linear(512, 9))
        # self.t2t_cls_head = ModuleList([cls_fc_head for _ in range(self.num_stages)])
