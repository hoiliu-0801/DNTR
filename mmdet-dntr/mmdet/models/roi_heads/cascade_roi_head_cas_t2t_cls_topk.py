...
class Cascade_t2t_cls_evit_RoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(Cascade_t2t_cls_evit_RoIHead, self).__init__(
        ...
        ###### tiny person dataset #####
        # self.t2t_cls_head = ModuleList([nn.Linear(evit_token*128, 2) for _ in range(self.num_stages)])
        ##########################
    ...
    def _bbox_forward(self, stage, x, rois):
        ...
        _, bbox_pred = bbox_head(bbox_feats)
        ...
        ########## evit topk ##########
        cls_token = self.cls_token[stage].expand(t2t_feats_cls.shape[0], -1, -1)
        # print(cls_token.shape, t2t_feats_cls.shape, t2t_feats_bbox.shape)
        feats_cls = torch.cat((cls_token, t2t_feats_cls), dim=1)
        feats_cls, cls_n_token, cls_token_idx = cls_blk(feats_cls)
        ################################
        # print(t2t_feats_cls.size())
        feats_cls = self.norm(feats_cls)
        # t2t_feats_cls = t2t_feats_cls[:, 0]
        feats_cls = torch.flatten(feats_cls, start_dim=1)
        # print(math.ceil(self.keep_rate*4) + 1)
        # print(t2t_feats_cls.size())
        cls_score = t2t_cls_head(feats_cls)

        # print("t2t_feats:", t2t_feats.size())     #torch.Size([1024, 4, 10])
        # t2t_feats_bbox = token_to_token_bbox(bbox_feats)
        ########## evit topk ##########
        # bbox_token = self.bbox_token[stage].expand(t2t_feats_bbox.shape[0], -1, -1)
        # feats_bbox = torch.cat((bbox_token, t2t_feats_cls, t2t_feats_bbox), dim=1)
        # feats_bbox, bbox_n_token, bbox_token_idx = bbox_blk(feats_bbox)
        ################################
        # feats_bbox = self.norm(feats_bbox)
        # t2t_feats_bbox = t2t_feats_bbox[:, 0]
        # feats_bbox = torch.flatten(feats_bbox, start_dim=1)
        # bbox_pred = t2t_bbox_head(feats_bbox)
