...
class Cascade_t2t_rand_shuf_RoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(Cascade_t2t_rand_shuf_RoIHead, self).__init__(
        ...
        # self.bbox_token = ModuleList([torch.rand(1024,1,128).to(torch.device('cuda:0')) for _ in range(self.num_stages)])
        # self.cls_token = ModuleList([torch.rand(1024,1,128).to(torch.device('cuda:0')) for _ in range(self.num_stages)])
        self.token_to_token_bbox = ModuleList([T2T_module(
                                img_size=7, tokens_type='performer', in_chans=256, embed_dim=128, token_dim=100) for _ in range(self.num_stages)])
        # self.t2t_bbox_head = nn.Linear(4*128, 4)
        self.t2t_bbox_head = ModuleList([nn.Linear(16*128, 4) for _ in range(self.num_stages)])
        self.token_to_token_cls = ModuleList([T2T_module(
                                img_size=7, tokens_type='performer', in_chans=256, embed_dim=128, token_dim=100) for _ in range(self.num_stages)])
        # self.t2t_cls_head = nn.Linear(4*128, 9)
        ###### aitod dataset #####
        self.t2t_cls_head = ModuleList([nn.Linear(16*128, 9) for _ in range(self.num_stages)])
        ##### visdrone dataset ###
        # self.t2t_cls_head = ModuleList([nn.Linear(4*128, 13) for _ in range(self.num_stages)])
        # crop
        # self.t2t_cls_head = ModuleList([nn.Linear(4*128, 11) for _ in range(self.num_stages)])
        ##########################
    ...
    def _bbox_forward(self, stage, x, rois):
        ...
        t2t_feats_cls = token_to_token_cls(bbox_feats, random_shuffle_forward=True)
        ...
        t2t_feats_bbox = token_to_token_bbox(bbox_feats, random_shuffle_forward=True)
