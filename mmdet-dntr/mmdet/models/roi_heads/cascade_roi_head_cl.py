...
#         if d == 0:
#             return torch.mean(torch.pow(dist, 2))  # distance squared
#         else:  # d == 1
#             delta = self.m - dist  # sort of reverse distance
...
class Contrastive_Loss(nn.Module):
    ...
    def forward(self,x):
        ...
        for i in range(scale_size):
            localization.append(self.loc_E(x[i]))
            semantic.append(self.sem_E(x[i]))
            ...
            for j in range(batch_size):
                ...
                for k in range(batch_size):
                    if k==j:continue
                        ...
                        numerator += self.cos(localization[i][j], localization[i][k])
        ...
        for i in range(scale_size):
            for j in range(batch_size):
                ...
                denominator += numerator
                sem_loss += -torch.log(torch.div(torch.div(numerator, denominator),4.))

        # loc_loss = torch.div(loc_loss, scale_size*batch_size)
        # sem_loss = torch.div(sem_loss, scale_size*batch_size)
...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        # Draw t-SNE picture
        # self.tsne_x = np.array(self.tsne_x)
        # tsne = TSNE(n_components=2, init="pca", learning_rate='auto')
        # z = tsne.fit_transform(self.tsne_x) 
        # df = pd.DataFrame()
        # df["y"] = self.tsne_y
        # df["comp-1"] = z[:,0]
        # df["comp-2"] = z[:,1]
