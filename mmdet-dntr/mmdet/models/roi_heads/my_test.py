...
class Attention(nn.Module):
    ...
    def dual_token_attn(q,k):
            ...
            q_cls[:,:,1,:] = 0
            k_cls[:,:,:,1] = 0
            Cls=q_cls@k_cls  # # [5, 8, 17, 16] * [5, 8, 16, 17] , q*k
            ...
            I,J=Cls.shape[-2:]
            final=torch.zeros(Cls.shape)
            for i in range(I):
                for j in range(J):
                    if (Cls[:,:,i][:,:,j]!=Box[:,:,i][:,:,j])[0,0]:
                        final[:,:,i,:][:,:,:,j]=Cls[:,:,i][:,:,j]+Box[:,:,i][:,:,j]
                    else:
                        if Cls[:,:,i][:,:,j]!=Box[:,:,i][:,:,j]:
                            print("Wrong")
                        else:
                            final_attn[:,:,i][:,:,j]=Cls[:,:,i][:,:,j]
            final_attn = final_attn.softmax(dim=-1)
            final_attn = self.attn_drop(attn)
            return  final_attn
...
class Cascade_test(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(Cascade_test, self).__init__(
        ...
        self.token_to_token = ModuleList([T2T_module(
                                img_size=7, tokens_type='transformer', in_chans=self.in_chans, embed_dim=self.embed_dim, token_dim=self.token_dim) for _ in range(self.num_stages)])
        self.t2t_bbox_head = ModuleList([nn.Linear(17*128, 4) for _ in range(self.num_stages)])
        ...
        ###### aitod dataset #####
        self.t2t_cls_head = ModuleList([nn.Linear(17*128, 9) for _ in range(self.num_stages)])
        ##### visdrone dataset ###
