...
class Cascade_t2t_evit_rand_shuf_forward_RoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(Cascade_t2t_evit_rand_shuf_forward_RoIHead, self).__init__(
        ...
        self.embed_dim = 128
        self.bbox_token = [nn.Paramter(torch.zeros(1,self.embed_dim)).to(torch.device('cuda:0')) for _ in range(self.num_stages)]
        self.cls_token = [nn.Parameter(torch.zeros(1,self.embed_dim)).to(torch.device('cuda:0')) for _ in range(self.num_stages)]
    ...
    def _bbox_forward(self, stage, x, rois):
        ...
        t2t_feats = token_to_token(bbox_feats, random_shuffle_forward=True)
