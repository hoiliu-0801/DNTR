...
class Cascade_t2t_dep_RoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(Cascade_t2t_dep_RoIHead, self).__init__(
        ...
        self.t2t_bbox_head = ModuleList([nn.Linear(4*128, 4) for _ in range(self.num_stages)])
        # self.token_to_token_cls = ModuleList([T2T_module(
        #                         img_size=7, tokens_type='performer', in_chans=256, embed_dim=128, token_dim=100) for _ in range(self.num_stages)])
        # self.t2t_cls_head = nn.Linear(4*128, 9)
        ###### aitod dataset #####
        self.t2t_cls_head = ModuleList([nn.Linear(4*128, 9) for _ in range(self.num_stages)])
        ##### visdrone dataset ###
    ...
    def _bbox_forward(self, stage, x, rois):
        ...
        t2t_feats = token_to_token(bbox_feats)
        t2t_feats = torch.flatten(t2t_feats, start_dim=1)
        # print("t2t_feats_cls:", t2t_feats_cls.size())
        cls_score = t2t_cls_head(t2t_feats)
        # print("t2t_feats:", t2t_feats.size())     #torch.Size([1024, 4, 10])
        bbox_pred = t2t_bbox_head(t2t_feats)
