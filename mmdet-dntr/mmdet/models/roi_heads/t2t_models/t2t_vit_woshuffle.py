...
class T2T_module(nn.Module):
    ...
    """
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, embed_dim=768, token_dim=64, mask=False):
        super().__init__()
    ...
    def forward(self, x, random_shuffle_forward=False, cls_token=None, bbox_token=None):
        ...
        # print("bbox_token:",bbox_token)
        if bbox_token is not None:
            x = torch.cat((bbox_token, x), dim=1)
        if cls_token is not None:
            x = torch.cat((cls_token, x), dim=1)

        x = self.attention2(x)                          #output size:(1024, 18, 100)
        ...
        B, new_HW, C = x.shape
        # print('split:', x.size())
        x = x.transpose(1, 2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))     #output size:(1024, 100, 4, 4)
