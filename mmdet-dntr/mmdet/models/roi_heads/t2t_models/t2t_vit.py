...
class T2T_module(nn.Module):
    ...
    def forward(self, x, random_shuffle_forward=False, cls_token=None, bbox_token=None):
        ...
        if random_shuffle_forward:
            #print(x.shape)  # torch.Size([512, 16, 2304])
            x = x.reshape(-1, 16, 256, 9).transpose(0, 3)
            #print(x.shape)  # torch.Size([9, 16, 256, 1024])
            ...
            #print(index)
            x = torch.cat((x[index[0]], x[index[1]], x[index[2]], x[index[3]]), dim=1)  # x[index[0]] 就是一直cat index[0]這個list
            #print(x[index[1]].shape)
            #print(x.shape)  # torch.Size([9, 64, 256, 1024])
            x = x.transpose(0, 3).reshape(-1,64,256*9)  
            # print(x.shape) #output size:(1024, 64, 2304)
        # print("split1:", x.size())
        # iteration2: re-structurization/reconstruction

        # 把box, class token 塞上去
        ...
        # 把 box, class token 取出來，過完self attention 再取出來，attention 中 cls, box的mask已經寫好了
        if cls_token is not None:
            cls_token = x[:, 0:1]
            x = x[:, 1:]
...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.tokens_to_token = T2T_module(
                img_size=img_size, tokens_type=tokens_type, in_chans=in_chans, embed_dim=embed_dim, token_dim=token_dim, mask=True)
